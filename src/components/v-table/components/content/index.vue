...
import { TableColumn as Column, TableColumnData } from "@arco-design/web-vue";

const components = { operations };

type Table = InstanceType<typeof _Table>;
type TableColumn = TableColumnData;
...
const getTableHeight = () => {
  ...
  const isCard = typeof config?.value?.card==='boolean'?config?.value?.card?32:0:32 ;
  ...
  return columns.map((column) => {
    ...
    if (!column.children && column.cellRender) {
      if (typeof column.cellRender === "function") {
        ...
        slot.cell = (record: any, column: TableColumnData, rowIndex: number) =>
